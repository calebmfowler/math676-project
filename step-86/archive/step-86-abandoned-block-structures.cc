...
namespace Step86
{
  ...
  private:
    ...
    static constexpr unsigned int             block_size = 2;
    const FESystem<dim>                       fe;
    DoFHandler<dim>                           dof_handler;

    IndexSet locally_owned_dofs;
    IndexSet locally_relevant_dofs;
    IndexSet::size_type num_locally_owned_dofs;
    IndexSet::size_type num_locally_relevant_dofs;
    ...
    void output_results(const double                      time,
                        ...
                        const PETScWrappers::MPI::BlockVector &solution);

    void implicit_function(const double                      time,
                           const PETScWrappers::MPI::BlockVector &solution,
                           const PETScWrappers::MPI::BlockVector &solution_dot,
    ...
    void solve_with_jacobian(const PETScWrappers::MPI::BlockVector &src,
                             PETScWrappers::MPI::BlockVector       &residual);
    ...
    void transfer_solution_vectors_to_new_mesh(
      const double                                   time,
      const std::vector<PETScWrappers::MPI::BlockVector> &all_in,
      std::vector<PETScWrappers::MPI::BlockVector>       &all_out);
  ...
  void HeatEquation<dim>::setup_system(const double time)
  {
    ...
    num_locally_owned_dofs = locally_owned_dofs.size();
    num_locally_relevant_dofs = locally_relevant_dofs.size();
    ...
    VectorTools::interpolate_boundary_values(dof_handler,
                                             0,
                                             Functions::ZeroFunction<dim>(block_size),
                                       ...
                                       PETScWrappers::MPI::BlockVector &residual)
  {
    ...
    std::vector<IndexSet> parallel_partitioning = {locally_owned_dofs, locally_owned_dofs};
    std::vector<IndexSet> ghost_indices = {locally_relevant_dofs, locally_relevant_dofs};
    
    PETScWrappers::MPI::BlockVector tmp_solution(parallel_partitioning,
                                            ghost_indices,
                                            mpi_communicator);
    PETScWrappers::MPI::BlockVector tmp_solution_dot(parallel_partitioning,
                                                ghost_indices,
                                                mpi_communicator);
    ...
    PETScWrappers::MPI::BlockVector locally_relevant_solution(parallel_partitioning,
                                                         ghost_indices,
                                                         mpi_communicator);
    PETScWrappers::MPI::BlockVector locally_relevant_solution_dot(
      parallel_partitioning, ghost_indices, mpi_communicator);
    locally_relevant_solution     = tmp_solution;
    locally_relevant_solution_dot = tmp_solution_dot;


    const QGauss<dim> quadrature_formula(fe.degree + 1);
    FEValues<dim, block_size> fe_values(fe,
                              quadrature_formula,
    ...
    std::vector<Tensor<block_size, dim, double>> solution_hessians(n_q_points); //<dim, block_size> or <block_size, dim>?
    std::vector<Vector<double>>         solution_dot_values(n_q_points);

    BlockVector<double> cell_residual(dofs_per_cell, block_size);
            ...
            for (const unsigned int i : fe_values.dof_indices())
              {
                cell_residual(i) +=
                  (fe_values.shape_value(i, q) *       // [phi_i(x_q) *
                     solution_dot_values[q]            //  dot u(x_q)
                   +                                   //  +
